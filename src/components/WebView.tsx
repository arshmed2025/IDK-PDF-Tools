...
const categoryColors: Record<ToolCategory, { bg: string, text: string, border: string, hover: string, glow: string }> = {
  ...
  'My Tools': {
    ...
    glow: 'dark:hover:shadow-fuchsia-900/20'
  }
}
...
export default function WebView({ tools }: { tools: Tool[] }) {
  ...
  const categories: (ToolCategory | 'All')[] = ['All', 'Edit', 'Secure', 'Convert', 'Optimize', 'My Tools']
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          <div className="max-w-2xl mx-auto relative group mt-12">
            ...
            <input 
              ...
              className="w-full bg-white dark:bg-zinc-900 border border-gray-100 dark:border-white/5 rounded-[2rem] py-6 pl-16 pr-8 shadow-2xl shadow-gray-200/50 dark:shadow-none focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 outline-none transition-all font-bold text-xl dark:text-white"
            />
          </div>
        </div>
      </section>
          ...
          <div className="flex-1">
            ...
            {filteredTools.length === 0 && (
              <div className="py-32 text-center">
                ...
                <button onClick={() => { setSearchQuery(''); setActiveCategory('All'); }} className="mt-8 text-sky-500 font-black uppercase tracking-widest text-xs hover:underline underline-offset-8">Reset Dashboard</button>
              </div>
            )}
          </div>
        </div>
